...
export default function EditPackage({ packagesObject, loading, tenantId }) {
  ...
  const getTotalQty = useMemo(() => getTotal('qty')?.toFixed(2), [mergedArray])
  const getTotalQtyToPack = useMemo(() => getTotal('packedQty')?.toFixed(2), [mergedArray])
    ...
    <React.Fragment>
      <PageHeader
        title={
          ...
          >
            Edit Package - {selectedPackage?.packageNo}
          </Typography>
        }
        button={
          <Box sx={{ display: 'flex', gap: 2, alignItems: 'center' }}>
            <Button
              ...
              component={Link}
              scroll={true}
              onClick={() => dispatch(setActionSalesOrder({}))}
                      ...
                      render={({ field }) => (
                        <CustomAutocomplete
                          ...
                          onChange={(event, newValue) => {
                            ...
                            // const filterData = newValue?.orderItems?.filter((value, index, self) => {
                            //   return self.findIndex(v => v.itemId === value.itemId) === index
                            // })
                            // setFilteredItem(filterData)
