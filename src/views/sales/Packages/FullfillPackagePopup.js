...
import { fulfillSalesOrderPackageMutation } from 'src/@core/components/graphql/sales-order-package-queries'
import { setUpdatePackage } from 'src/store/apps/packages'

export default function FullfillPackagePopup({ tenantId, open, setOpen }) {
  ...
  const handleFormSubmit = async data => {
    ...
    try {
      const response = await writeData(fulfillSalesOrderPackageMutation(), { tenantId, packageId, packedByUsername })

      if (response.fulfillSalesOrderPackage) {
        dispatch(setUpdatePackage(response.fulfillSalesOrderPackage))
        dispatch(createAlert({ message: 'Package fulfilled  successfully !', type: 'success' }))
        setOpen(false)
      } else {
        dispatch(createAlert({ message: 'Failed to fullfill package !', type: 'error' }))
        setOpen(false)
      }
      return response
    } catch (error) {
