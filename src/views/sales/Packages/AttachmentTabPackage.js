...
import {
  attachFileToPackageQuery,
  deleteFileFromPackage
...
export default function AttachmentTabPackage({ salesPackage, folderName }) {
  ...
  useEffect(() => {
    if (salesPackage?.files?.length > 0) {
      setSelectedPdFile([])
      salesPackage?.files?.map(item => {
  ...
  const handleUploadFiles = async () => {
    ...
    try {
      const response = await writeData(attachFileToPackageQuery(), {
        tenantId: salesPackage.tenantId,
        packageId: salesPackage.packageId,
        files: filePathsArray
      })
      if (response.attachFileToPackage) {
        await UploadMultipleFileS3Api(selectedFilesToUpload, dispatch)

        dispatch(setUpdatePackage({ ...salesPackage, files: response.attachFileToPackage }))
        dispatch(setSelectedPackages({ ...salesPackage, files: response.attachFileToPackage }))
        setSelectedPdFile(response.attachFileToPackage)
  ...
  const removeFile = async file => {
    setLoading(true)
    try {
      const response = await writeData(deleteFileFromPackage(), {
        tenantId: salesPackage.tenantId,
        packageId: salesPackage.packageId,
        file
      })

      if (response.deleteFileFromPackage) {
        await DeleteUploadFile(file.key)
        dispatch(setUpdatePackage({ ...salesPackage, files: response.deleteFileFromPackage }))
        dispatch(setSelectedPackages({ ...salesPackage, files: response.deleteFileFromPackage }))
        setSelectedPdFile(response.deleteFileFromPackage)
