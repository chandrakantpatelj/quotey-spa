...
function DeleteCustomerNote({ selectedNotes, setOpenDialog, openDialog, vendorNotesData, setCustomerNotesData }) {
  const dispatch = useDispatch()

  const handleConfirm = async () => {
    const previousNotesData = vendorNotesData
    setOpenDialog(false)

    try {
      setCustomerNotesData(prevData => prevData.filter(note => note.noteId !== selectedNotes.noteId))
      ...
      if (response.deleteStakeholderNote) {
        ...
        setCustomerNotesData(previousNotesData)
      }
    } catch (error) {
      setOpenDialog(false)
      dispatch(
