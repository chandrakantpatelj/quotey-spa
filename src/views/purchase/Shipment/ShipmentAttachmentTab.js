...
import {
  attachFilesToPurchaseOrderShipment,
  deleteFileFromPurchaseOrderShipment
...
export default function ShipmentAttachmentTab({ order, folderName }) {
  ...
  const handleUploadFiles = async () => {
    ...
    try {
      const response = await writeData(attachFilesToPurchaseOrderShipment(), {
        tenantId: order.tenantId,
        shipmentId: order.shipmentId,
        files: filePathsArray
      })

      if (response.attachFilesToPurchaseOrderShipment) {
        await UploadMultipleFileS3Api(selectedFilesToUpload, dispatch)

        dispatch(setUpdatePurchaseShipment({ ...order, files: response.attachFilesToPurchaseOrderShipment }))
  ...
  const removeFile = async file => {
    setLoading(true)
    try {
      const response = await writeData(deleteFileFromPurchaseOrderShipment(), {
        tenantId: order.tenantId,
        shipmentId: order.shipmentId,
        file
      })

      if (response.deleteFileFromPurchaseOrderShipment) {
        await DeleteUploadFile(file.key)
        dispatch(setUpdatePurchaseShipment({ ...order, files: response.deleteFileFromPurchaseOrderShipment }))
