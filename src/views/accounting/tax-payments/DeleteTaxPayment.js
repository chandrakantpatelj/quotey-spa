...
export default function DeleteTaxPayment({ tenantId, statementId, setTaxesData, openDialog, setOpenDialog }) {
  ...
  const handleConfirm = async () => {
    try {
      ...
      if (response.deleteTaxStatement) {
        setTaxesData(prevData => ({
          ...prevData,
          taxStatements: prevData.taxStatements.filter(item => item.statementId !== statementId)
        }))
        // dispatch(setDeleteExpense(statementId))
